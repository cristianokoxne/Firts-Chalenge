...
Entregas *rotateLeft(Entregas *tree)
{
...
Entregas *rotateRight(Entregas *tree)
{
...
Entregas *inserirEntregas(Entregas *tree, int key, bool *grown)
{
    if (tree == NULL)
    {
        tree = CriarEntregas(key);
        *grown = true; //ter controle caso a árvore crescer
    }
    else if (key < tree->key)
    {
        tree->left = inserirEntregas(tree->left, key, grown);
    ...
    {
        tree->right = inserirEntregas(tree->right, key, grown);
